...
public class CountyCodesHandler implements Route {
  /**
...
//    String participants = request.queryParams("participants");
    ...
    try {
      ...
      String activityJson = this.sendRequest("NAME", "county:*");
      // Deserializes JSON into an Activity
//      Activity activity = ActivityAPIUtilities.deserializeActivity(activityJson);
      // Adds results to the responseMap
      responseMap.put("result", "success");
      responseMap.put("activity", CensusAPIUtilities.deserializeCountyCodes(activityJson));
      return responseMap;
    } catch (Exception e) {
      e.printStackTrace();
  ...
  private String sendRequest(String get, String county) throws URISyntaxException, IOException, InterruptedException {
    ...
    HttpRequest buildBoredApiRequest =
        HttpRequest.newBuilder()
            .uri(new URI("https://api.census.gov/data/2010/dec/sf1?get=" + get + "&for=" + county))
            .GET()
            .build();
